...
final class ATAC_Dashboard_Widgets {
	...
	public function maybe_save_settings() {
		...
		add_settings_error(
			...
			'atac_saved',
			__( 'Dashboard widget settings saved.', 'adriantoro-admin-palette' ),
			'success'
		);
	}
		...
		<form method="post" action="">
			...
			<p>
				<label>
					...
					<?php echo esc_html__( 'Hide default WordPress dashboard widgets', 'adriantoro-admin-palette' ); ?>
				</label>
			</p>
			...
			<?php foreach ( $settings['boxes'] as $index => $box ) : ?>
				<fieldset class="atac-box-fieldset">
					<legend>
						...
						echo esc_html(
							sprintf(
								...
								__( 'Custom box %d', 'adriantoro-admin-palette' ),
								$index + 1
							)
						);
						?>
					...
					<p>
						<label>
							<?php echo esc_html__( 'Title', 'adriantoro-admin-palette' ); ?><br />
							<input type="text" class="regular-text" name="<?php echo esc_attr( 'atac_box_' . $index . '_title' ); ?>" value="<?php echo esc_attr( $box['title'] ); ?>" />
						</label>
					</p>
					<p>
						<label><?php echo esc_html__( 'Content', 'adriantoro-admin-palette' ); ?></label>
					</p>
					<?php
					wp_editor(
						$box['content'],
					...
					?>
					<p><?php echo esc_html__( 'Visible to roles:', 'adriantoro-admin-palette' ); ?></p>
			...
			<?php submit_button( __( 'Save widget settings', 'adriantoro-admin-palette' ) ); ?>
		</form>
		<?php
	}
	...
	public function register_widgets() {
		...
		foreach ( $settings['boxes'] as $index => $box ) {
			...
			wp_add_dashboard_widget(
				$id,
				$box['title'] ? $box['title'] : __( 'Custom box', 'adriantoro-admin-palette' ),
				function () use ( $box ) {
